...
export function createEditorWindow(): BrowserWindow {
	...
	const win = new BrowserWindow({
		...
		webPreferences: {
			...
			nodeIntegration: false,
			contextIsolation: true,
			// Electron defaults sandbox:true on all renderers since v20, which
			// blocks node: imports in the preload (incl. node:net used by the
			// fast TCP encoder transport). The editor window only loads our
			// own bundled code — no third-party iframes, no remote HTML — so
			// disabling sandbox here is a calculated tradeoff for the encoder
			// throughput. contextIsolation stays on, so the renderer page's
			// JS still can't reach Node directly; only the preload can, and
			// only what we whitelist via contextBridge gets exposed.
			sandbox: false,
			webSecurity: false,
			backgroundThrottling: false,
		},
	...
	// Auto-open DevTools in dev so renderer logs are reachable without keyboard
	// hunting. F12 / Ctrl+Shift+I still works manually too.
	if (VITE_DEV_SERVER_URL) {
