...
import { registerVideoEncoderHandlers } from "./videoEncoderHandlers";
...
const PROJECT_FILE_EXTENSION = "cinder";
...
export function registerIpcHandlers(
	...
	/**
	 * Auto-save an exported video to a predictable per-OS path without showing
	 * a save dialog. Writes to <Videos>/Cinder/<fileName>. Returns a
	 * canceled-style failure if the directory can't be created or the write
	 * fails so the renderer can fall back to the dialog flow.
	 */
	ipcMain.handle(
		"auto-save-exported-video",
		async (_, videoData: ArrayBuffer, fileName: string) => {
			try {
				const videosDir = app.getPath("videos");
				const targetDir = path.join(videosDir, "Cinder");
				await fs.mkdir(targetDir, { recursive: true });

				// Avoid overwriting an existing file at the same exact path —
				// add a -N suffix if needed. Filenames are timestamp-based so
				// collisions are rare, but be safe.
				...
				console.error("Failed to auto-save exported video:", error);
				return {
					success: false,
		...
		async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string) => {
			try {
				...
				const result = await dialog.showSaveDialog({
					...
					filters: [
						{
							name: mainT("dialogs", "fileDialogs.cinderProject"),
