...
// Hybrid-graphics laptops (e.g. NVIDIA Optimus) default Chromium to the Intel
// iGPU, which makes WebGL readback for the export pipeline ~5× slower than the
// dGPU and creates a cross-GPU bounce when ffmpeg encodes on NVENC. Request
// the discrete GPU explicitly. Windows GPU prefs in Settings can still override.
app.commandLine.appendSwitch("force_high_performance_gpu");
// Belt-and-suspenders: also enable Chromium's discrete-GPU heuristic.
app.commandLine.appendSwitch("enable-features", "UseHighPerformanceGPUForWebGL");
// ANGLE backend selection. Empirically D3D11 (Chromium's Windows default)
// gives the highest peak fps for our worker-pool export pipeline — Vulkan
// stabilized fps but lost the bursts. Override via CINDER_ANGLE env
// (values: gl | vulkan | d3d11 | metal) if a specific user's hardware
// behaves differently.
const angleBackend = process.env.CINDER_ANGLE;
if (angleBackend) {
	app.commandLine.appendSwitch("use-angle", angleBackend);
	console.log(`[main] ANGLE backend override: ${angleBackend}`);
}

export const RECORDINGS_DIR = path.join(app.getPath("userData"), "recordings");

// Mirror renderer console output into the main-process terminal. Preload sends
// these via `renderer-log`; we just print them with a clear prefix so devs can
// tail one stream instead of fishing in DevTools.
ipcMain.on(
	"renderer-log",
	(_event, level: "log" | "warn" | "error" | "info" | "debug", args: string[]) => {
		const tag = `[35m[renderer][0m`;
...
// Tray Icons
const defaultTrayIcon = getTrayIcon("cinder.png", trayIconSize);
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Cinder";
