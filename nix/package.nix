...
buildNpmPackage {
  ...
  installPhase = ''
    ...
    mkdir -p "$out/lib/cinder/public"
    cp -r public/wallpapers "$out/lib/cinder/public/wallpapers"

    # Wrap system electron with the app directory
    mkdir -p "$out/bin"
    makeWrapper "${electron}/bin/electron" "$out/bin/cinder" \
      --add-flags "$out/lib/cinder" \
    ...
    for size in 16 24 32 48 64 128 256 512 1024; do
      ...
      if [ -f "$icon" ]; then
        install -Dm644 "$icon" \
          "$out/share/icons/hicolor/''${size}x''${size}/apps/cinder.png"
      fi
    done
  ...
  meta = {
    ...
    homepage = "https://github.com/talktodre-ops/Cinder";
