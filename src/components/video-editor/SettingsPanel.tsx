														...
														try {
															...
															if (!result.granted) {
																toast.message("Accessibility permission needed", {
																	description:
																		"Open System Settings → Privacy & Security → Accessibility, enable Cinder, then restart the app.",
																});
															}
														} catch (err) {
			...
			<div className="flex-shrink-0 p-4 pt-3 border-t border-white/5 bg-[#09090b]">
				...
				<div className="flex gap-2 mt-3">
					<button
						...
						onClick={() => {
							window.electronAPI?.openExternalUrl(
								"https://github.com/talktodre-ops/cinder/issues/new/choose",
							);
						}}
					...
					<button
						type="button"
						onClick={() => {
							window.electronAPI?.openExternalUrl("https://github.com/talktodre-ops/cinder");
						}}
						className="flex-1 flex items-center justify-center gap-1.5 text-[10px] text-slate-500 hover:text-slate-300 py-1.5 transition-colors"
					>
