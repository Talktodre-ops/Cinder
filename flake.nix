{
  description = "Cinder — desktop screen recorder with built-in editor";
  ...
  outputs =
    ...
    in
    {
      ...
      packages = forAllSystems (pkgs: {
        cinder = pkgs.callPackage ./nix/package.nix { };
      ...
      devShells = forAllSystems (
        ...
        in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "Cinder dev shell — node $(node --version), electron v$(electron --version 2>/dev/null | tr -d 'v')"
            '';
          };
        }
      );
      ...
      overlays.default = final: _prev: {
        cinder = self.packages.${final.stdenv.hostPlatform.system}.cinder;
      };
